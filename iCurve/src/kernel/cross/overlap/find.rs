...
pub(crate) trait OverlapSegment<T: FloatNumber>: PointAt<T> {
...
impl<T: FloatNumber> OverlapSegment<T> for QuadSegment<T> {
    #[inline]
    fn first_point(&self) -> FloatPoint<T> {
...
impl<T: FloatNumber> OverlapSegment<T> for CubicSegment<T> {
    #[inline]
    fn first_point(&self) -> FloatPoint<T> {
        self.control_points[0]
    }
...
fn is_valid_overlap<T: FloatNumber, S: OverlapSegment<T>>(
    ...
    for i in 1..=sample_count {
        ...
        let p0 = segment_0.point_at(SegmentParam::from(t0));
        let p1 = segment_1.point_at(SegmentParam::from(t1));
