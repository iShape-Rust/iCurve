...
impl<T: FloatNumber> Solver<T> {
    pub fn intersect_line_and_line(
        &mut self,
        line0: FloatLineSegment<T>,
        line1: FloatLineSegment<T>,
        ...
        if !FloatSegmentParam::is_in_unit_range(t, self.grid_size())
            || !FloatSegmentParam::is_in_unit_range(u, self.grid_size())
        {
            return;
        }

        let t = FloatSegmentParam::clamp_unit(t);
        let u = FloatSegmentParam::clamp_unit(u);

        output.push(ContactPoint {
            point: a0 + r * t,
            t0: FloatSegmentParam::from(t),
            t1: FloatSegmentParam::from(u),
        });
    }
